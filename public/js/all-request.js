...
function updateChap() {
  ...
  save.addEventListener("click", function (e) {
    ...
    $.ajax({
      type: "POST",
      url: "http://localhost/projet4/public/index.php?action=deleteChap",
      data: {
        chap: chapitre,
        contenu: contenu,
        titre: titre,
...
//       success: function (code_html, statut) {
//         //window.location.href = "#mssg";
//         html("<p class = 'mssg'>Le chapitre a bien été supprimer !</p>");
//       },
//       error: function (resultat, statut, erreur) {
//         // window.location.href = "#mssg";
//         html(
...
function reportComm() {
  ...
  if (signaler != null && signaler.length < 1) {
    for (let i = 0; i < signaler.length; i++) {
